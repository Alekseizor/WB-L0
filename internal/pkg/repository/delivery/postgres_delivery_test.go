...
func TestAddDelivery(t *testing.T) {
	...
	for caseNum, item := range testCases {
		...
		if err != item.err {
			t.Errorf("[%d] the error %d  is different from the expected one %d", caseNum, err, item.err)
		}
	}
...
func TestGetDeliveryByUUID(t *testing.T) {
	...
	mock.ExpectQuery("SELECT * FROM delivery WHERE delivery_uuid=").WillReturnRows(sqlmock.NewRows(result).AddRow(uuidTest, "name", "phone", "zip", "city", "address", "region", "email"))
	mock.ExpectQuery("SELECT * FROM delivery WHERE delivery_uuid=").WillReturnError(errTest)
	for caseNum, item := range testCases {
